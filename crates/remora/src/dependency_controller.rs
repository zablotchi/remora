...
impl Default for DependencyController {
    fn default() -> Self {
        Self::new()
    }
}
...
impl DependencyController {
    ...
    pub fn get_dependencies(
        ...
        let current_handles: Vec<_> = (0..obj_ids.len())
            .map(|_| Arc::new(Notify::new()))
            .collect();

        for (&obj_id, notify) in obj_ids.iter().zip(current_handles.iter()) {
...
mod tests {
    ...
    fn test_no_prior_dependencies() {
        let mut dependency_controller = DependencyController::new();
        let task_id = 1;
        let obj_ids = vec![ObjectID::random(), ObjectID::random()];

        let (prior_tasks, current_tasks) =
            dependency_controller.get_dependencies(task_id, obj_ids.clone());
    ...
    #[test]
    fn test_with_prior_dependencies_same_object() {
        let mut dependency_controller = DependencyController::new();
        let task_id1 = 1;
        let task_id2 = 2;
        ...
        let (prior_tasks, current_tasks1) =
            dependency_controller.get_dependencies(task_id1, vec![obj_id]);
        assert!(
            prior_tasks.is_empty(),
        ...
        let (prior_tasks, current_tasks2) =
            dependency_controller.get_dependencies(task_id2, vec![obj_id]);
        assert_eq!(
            prior_tasks.len(),
            1,
    ...
    #[test]
    fn test_multiple_objects_no_overlap() {
        let mut dependency_controller = DependencyController::new();
        let task_id1 = 1;
        let task_id2 = 2;
        ...
        let (prior_tasks1, current_tasks1) =
            dependency_controller.get_dependencies(task_id1, obj_ids1.clone());
        assert!(
        ...
        let (prior_tasks2, current_tasks2) =
            dependency_controller.get_dependencies(task_id2, obj_ids2.clone());
        assert!(
    ...
    #[test]
    fn test_partial_prior_dependencies() {
        let mut dependency_controller = DependencyController::new();
        let task_id1 = 1;
        let task_id2 = 2;
        let obj_ids1 = vec![ObjectID::random(), ObjectID::random()];
